...
it('pipes buffer through encoder and decoder', async () => {
    ...
    expect(payload).toEqual(buffer.toString());
});
...
describe('decode()', () => {
    it('decodes a short buffer (1)', () => {
        ...
        expect(B64.decode(encoded).toString()).toEqual(value);
    });
...
describe('Encoder', () => {
    it('process remainder', async () => {
        ...
        const payload = await internals.test(buffer);
        expect(payload).toEqual(Buffer.concat(buffer).toString());
    });
...
describe('Decoder', () => {
    it('process remainder', async () => {
        ...
        expect(payload.toString()).toEqual(value.toString());
    });
...
describe('Base64Url', () => {
    ...
    describe('base64urlEncode()', () => {
        ...
        it('should base64 URL-safe a string', () => {
            expect(B64.base64urlEncode(str)).toEqual(base64str);
        });

        it('encodes a buffer', () => {
            expect(B64.base64urlEncode(Buffer.from(str, 'binary'))).toEqual(base64str);
        });

        it('should base64 URL-safe a hex string', () => {
            const buffer = Buffer.from(str, 'binary');
            expect(B64.base64urlEncode(buffer.toString('hex'), 'hex')).toEqual(base64str);
        });

        it('works on larger input strings', () => {
            const input = Fs.readFileSync(Path.join(import.meta.dirname, 'index.js')).toString();
    ...
    describe('base64urlDecode()', () => {
        it('should un-base64 URL-safe a string', () => {
            expect(B64.base64urlDecode(base64str)).toEqual(str);
        });

        it('should un-base64 URL-safe a string into hex', () => {
            expect(B64.base64urlDecode(base64str, 'hex')).toEqual(Buffer.from(str, 'binary').toString('hex'));
        });

        it('should un-base64 URL-safe a string and return a buffer', () => {
            const buf = B64.base64urlDecode(base64str, 'buffer');
            expect(buf instanceof Buffer).toEqual(true);
            expect(buf.toString('binary')).toEqual(str);
        });

        it('throws error on invalid input', () => {
            expect(() => B64.base64urlDecode(1024)).toThrow('Value not a string');
        });

        it('throws error on invalid input', () => {
            expect(() => B64.base64urlDecode('*')).toThrow('Invalid character');
        });
    });
});
